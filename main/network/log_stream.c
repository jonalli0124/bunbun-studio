...
static esp_err_t ws_log_handler(httpd_req_t *req) {
   ...
   * misreported as "not properly masked", failing the handler.
   *
   * Recv ERRORS must FAIL the handler: returning ESP_OK for a dead socket
   * keeps the session alive and httpd re-invokes this handler in a tight
   * loop, flooding the log with sock_err/masking warnings at tens of Hz —
   * the log viewer drowning the log, at exactly the field-debugging moment
   * it exists for (2026-08-05, phone hopping off a hotspot). ESP_FAIL makes
   * httpd close the session; the viewer reconnects with a fresh one. */
  httpd_ws_frame_t frame = {0};
  if (httpd_ws_recv_frame(req, &frame, 0) != ESP_OK) {
    return ESP_FAIL; /* dead/garbled socket: hang up, don't spin */
  }
  if (frame.type == HTTPD_WS_TYPE_CLOSE) {
    return ESP_FAIL; /* the browser said goodbye: let httpd close us */
  }
  if (frame.len > 0) {
    uint8_t buf[128];
    if (frame.len > sizeof(buf)) {
      return ESP_FAIL; /* can't drain it aligned, so don't parse mid-frame */
    }
    frame.payload = buf;
