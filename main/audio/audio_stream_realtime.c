...
// The receiver task's stack lives in PSRAM (W-042, 2026-08-07, brother's-
// house serial session). The measured failure: on busy networks the WiFi
// driver's dynamic RX buffers sag internal RAM until no 12288-byte
// contiguous block exists at RECORD (boot baseline: largest 23552; by
// RECORD: largest 10240, every time) — AirPlay could never start playback
// away from quiet home networks. PSRAM has megabytes free and no
// fragmentation problem; mDNS already runs its task from SPIRAM, the fw
// checker ran full TLS handshakes on a PSRAM stack the same night, and
// this task touches no flash-driver ops, so the stack is legal there.
// Static TCB (internal .bss); the 12KB buffer is allocated from PSRAM once
// and reused — sessions are serial. Internal-stack fallback kept in case
// the PSRAM alloc ever fails, preserving the old behavior exactly.
static StaticTask_t s_recv_tcb;
static StackType_t *s_recv_stack_psram = NULL;
// The control receiver task's stack ALSO lives in PSRAM (8/11: as the pet
// grew, its 4KB internal stack could no longer be allocated at PLAY, so
// SETUP failed with "Failed to create control receiver task" and AirPlay
// dropped the moment you hit play). Same cure the main receiver already
// uses - reserved once, reused every session.
static StaticTask_t s_ctrl_tcb;
static StackType_t *s_ctrl_stack_psram = NULL;
...
static esp_err_t realtime_start(audio_stream_t *stream, uint16_t port) {
  ...
  state->task_handle = NULL;
  if (!s_recv_stack_psram) {
    s_recv_stack_psram = heap_caps_malloc(
        AUDIO_RECV_STACK_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  }
  if (s_recv_stack_psram) {
    state->task_handle = xTaskCreateStaticPinnedToCore(
        receiver_task, "audio_recv", AUDIO_RECV_STACK_SIZE, stream, 8,
        s_recv_stack_psram, &s_recv_tcb, AUDIO_TASK_CORE);
  } else {
  ...
  if (state->control_socket > 0) {
    state->control_task_handle = NULL;
    if (!s_ctrl_stack_psram) {
      s_ctrl_stack_psram = heap_caps_malloc(
          AUDIO_CTRL_STACK_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    }
    if (s_ctrl_stack_psram) {
      state->control_task_handle = xTaskCreateStaticPinnedToCore(
          control_receiver_task, "ctrl_recv", AUDIO_CTRL_STACK_SIZE, stream, 7,
          s_ctrl_stack_psram, &s_ctrl_tcb, AUDIO_TASK_CORE);
    } else {
      BaseType_t task_ret = xTaskCreatePinnedToCore(
          control_receiver_task, "ctrl_recv", AUDIO_CTRL_STACK_SIZE, stream, 7,
          &state->control_task_handle, AUDIO_TASK_CORE);
      if (task_ret != pdPASS) state->control_task_handle = NULL;
    }
