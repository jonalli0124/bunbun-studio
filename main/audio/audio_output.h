...
/**
 * The I2S RX channel for the codec's record path (ES8311 onboard mic).
 * Created ON DEMAND (its DMA lives in scarce internal RAM) and returned
 * enabled; NULL on boards without CONFIG_I2S_DI_IO or on failure. Call
 * audio_output_rx_release() when the capture ends to give the memory back.
 */
...
/**
 * Millisecond timestamp of the last frame of REAL stream audio reaching the
 * speaker (0 = never since boot). The honest "is music playing" signal —
 * audio_receiver_is_playing() reports intent and can be true on an idle unit.
 */
uint32_t audio_output_last_stream_ms(void);
