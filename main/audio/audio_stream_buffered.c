...
static esp_err_t buffered_start(audio_stream_t *stream, uint16_t port) {
  ...
  // Stack in PSRAM (8/11): the AAC/AirPlay-2 path is the modern one, and its
  // 4KB internal stack fails to allocate for the same reason the ALAC path's
  // did once the pet grew - so the fleet must not depend on that 4KB being
  // free at PLAY. The task decodes+outputs and touches no flash, so PSRAM is
  // safe. Reserved once, reused each session.
  static StaticTask_t s_buff_tcb;
  static StackType_t *s_buff_stack = NULL;
  state->buffered_task_handle = NULL;
  if (!s_buff_stack) {
    s_buff_stack = heap_caps_malloc(AUDIO_BUFFERED_STACK_SIZE,
                                    MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  }
  ...
  if (s_buff_stack) {
    state->buffered_task_handle =
        xTaskCreateStatic(buffered_audio_task, "buff_audio",
                          AUDIO_BUFFERED_STACK_SIZE, stream, 5, s_buff_stack,
                          &s_buff_tcb);
  } else {
    task_ret = xTaskCreate(buffered_audio_task, "buff_audio",
