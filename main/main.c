...
// bunbun (components/bunbun): the virtual pet that owns the display and touch panel.
extern void bunbun_start(void);
...
static void start_airplay_services(void) {
  ...
  // W-060: if this image is fresh from an OTA it is PENDING_VERIFY right
  // now - open the validation window. Long-blessed images no-op.
  fw_selfcheck_begin();
}
#ifdef CONFIG_BT_A2DP_ENABLE
...
void app_main(void) {
  ...
  // W-020: a boot flagged as USB card-reader mode becomes a USB drive and
  // NOTHING else — no WiFi, no AirPlay, no pet. Checked this early so the
  // RAM those subsystems would take is simply never spent. Never returns.
  if (usb_msc_mode_flagged()) {
    usb_msc_mode_run();
  }
  spiffs_storage_init();
  log_stream_init();
  ...
  // The pet starts HERE, before ethernet and WiFi, and the reason is memory rather than tidiness.
  // bunbun composites into a 240x180x16bpp sprite — 86,400 CONTIGUOUS bytes. Started after the
  // network and audio stacks, the internal heap still had 124KB free but its largest block was
  // only 51KB, so TFT_eSPI silently fell back to PSRAM. Every frame then composited across the
  // same octal PSRAM bus the audio pipeline is saturating, and the render loop decayed from
  // 90Hz to a hard 10Hz as those buffers filled.
  //
  // Nothing it needs at construction is network-dependent: the display, touch panel, RTC and
  // codec are all up by now, and net.h reports NET_OFF gracefully until WiFi arrives.
  bunbun_start();

  // NOTE (2026-08-07): audio init deliberately stays HERE-gated (network-up,
  // inside start_airplay_services) and NOT at boot. Moving it to boot fixed
  // AP-mode mutism but rearranged the internal heap enough that the AirPlay
  // receiver task (12KB contiguous) lost to fragmentation on real networks —
  // "Failed to create receiver task" on every RECORD, field-caught same day.
  // A static-stack rescue made things worse (likely starved the bunbun
  // sprite's 86KB contiguous claim). The AP-voice wish is still wanted, but
  // it needs a bench session with heap tracing, not a remote guess. Music
  // outranks setup chirps.
