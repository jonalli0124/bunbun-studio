...
static void server_task(void *pvParameters) {
  ...
  while (server_running) {
    ...
    // Start new client task immediately, on a PSRAM stack (8/12). Its 8KB
    // INTERNAL stack was the last AirPlay allocation still fighting for scarce
    // internal RAM, and on units fragmented below 8KB it failed at every
    // connect ("Failed to create client task" - D424/5DC0 dead, D468 alive
    // only by a 1KB margin). The task no longer touches flash (device name is
    // cached in RAM now), so PSRAM is safe. One reserved stack + TCB per slot.
    static StaticTask_t s_client_tcb[2];
    static StackType_t *s_client_stack[2] = {NULL, NULL};
    clients[new_slot].task = NULL;
    if (new_slot < 2 && !s_client_stack[new_slot]) {
      s_client_stack[new_slot] = heap_caps_malloc(
          CLIENT_STACK_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    }
