...
static int rtsp_create_event_socket(uint16_t *port) {
  uint16_t bound_port = 0;
  // Non-blocking listener (W-013): with a blocking one, select() can report
  // a pending connection that the client aborts before accept() runs — and
  // accept() then blocks forever. shutdown() on a LISTENING socket is a
  // no-op in lwip, so the stop path couldn't wake it: the task leaked its
  // stack and two sockets per occurrence, fragmenting internal RAM until an
  // AirPlay session failed to start (field, 2026-08-05, largest block 8KB).
  int sock = socket_utils_bind_tcp_listener(0, 1, true, &bound_port);
  if (sock < 0) {
    return -1;
  }
  if (port) {
    ...
    if (FD_ISSET(listen_socket, &read_fds)) {
      ...
      if (client < 0) {
        if (errno == EWOULDBLOCK || errno == EAGAIN) {
          continue; // aborted between select() and accept() — just re-poll
        }
        if (!event_task_should_stop) {
...
esp_err_t rtsp_start_event_port_task(int listen_socket) {
  if (event_task_handle != NULL) {
    ...
    if (!event_port_wait_for_task_stopped(60)) {
  ...
  event_listen_socket = -1;
  event_task_handle = NULL;
  // PSRAM stack (8/11): the AirPlay event channel touches no flash, so free
  // its 3KB from the scarce internal pool that AirPlay SETUP fights over.
  static StaticTask_t s_event_tcb;
  static StackType_t *s_event_stack = NULL;
  if (!s_event_stack) {
  ...
  if (s_event_stack) {
    ...
    ret = xTaskCreate(event_port_task, "event_port", EVENT_STACK_SIZE,
                      (void *)(intptr_t)listen_socket, 5, &event_task_handle);
  }
  if (ret != pdPASS || !event_task_handle) {
    event_task_handle = NULL;
...
void rtsp_stop_event_port_task(void) {
  ...
  // 3s, not 1s: the task's select() polls at 1s, so a 1s wait raced the
  // task's own worst-case wake-up and logged "did not exit" about healthy
  // exits — while the next SETUP built a second event port alongside.
  if (!event_port_wait_for_task_stopped(60)) {
...
static void handle_get(int socket, rtsp_conn_t *conn, const rtsp_request_t *req,
  ...
  if (strcmp(req->path, "/info") == 0) {
    ...
    // Senders re-read features from /info during connection, so this must
    // agree with the mDNS advertisement — classic (hotspot/V1) or AirPlay 2.
    bool classic = airplay_classic_mode();
    uint64_t features =
        classic
            ? (((uint64_t)AIRPLAY_FEATURES_V1_HI << 32) | AIRPLAY_FEATURES_V1_LO)
            : (((uint64_t)AIRPLAY_FEATURES_V2_HI << 32) | AIRPLAY_FEATURES_V2_LO);
    int64_t protocol_version = classic ? 1 : 2;
...
static void handle_set_parameter(int socket, rtsp_conn_t *conn,
  ...
  if (has_metadata) {
    // bunbun's now-playing ticker. This one call site covers BOTH protocol generations — the
    // AirPlay 1 DMAP branch and the AirPlay 2 bplist branch both land here — which is exactly
    // the metadata the old RAOP receiver never delivered (its parser demanded an exact
    // Content-Type match and dropped everything).
    extern void bunbun_now_playing(const char *artist, const char *title);
    bunbun_now_playing(event_data.metadata.artist, event_data.metadata.title);
