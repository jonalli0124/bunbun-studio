...
// Flight recorder: dump the registers that decide whether sound exists, so a
// silent-in-the-field unit can testify over HTTP instead of being theorized
// about. Read-only; safe any time.
int dac_es8311_reg_dump(char *out, size_t cap) {
  // 0x15 and 0x18-0x1B are the ADC soft-ramp + ALC (auto level control) bank
  // — added 8/12: it was never dumped, and an ALC left at chip defaults is
  // the prime suspect for the mic's silent capture-to-capture gain swings.
  static const uint8_t regs[] = {0x00, 0x01, 0x02, 0x05, 0x06, 0x07, 0x08,
                                 0x09, 0x0A, 0x0D, 0x0E, 0x12, 0x13, 0x14,
                                 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
                                 0x1C, 0x31, 0x32, 0x37};
  int n = 0;
  for (size_t i = 0; i < sizeof(regs) && (size_t)n < cap - 12; i++) {
    uint8_t v = 0xEE;
    for (int attempt = 0; attempt < 3; attempt++) {
      if (board_i2c_read(s_es8311_device, regs[i], &v, 1) == ESP_OK) {
        break;
      }
      vTaskDelay(pdMS_TO_TICKS(2));
    }
    n += snprintf(out + n, cap - n, "%s%02X=%02X", i ? " " : "", regs[i], v);
  }
  return n;
}
...
static esp_err_t es8311_program(void);
...
static esp_err_t es8311_init(void *i2c_bus) {
  ...
  return es8311_program();
}

// The full soft-reset + register program, split out of es8311_init so it can
// be re-run at CAPTURE time (dac_es8311_capture_reset below). The W-068 mic
// lab's core finding — a codec recording full-scale hiss while its registers
// read back byte-identical to a healthy unit — means the ANALOG input stage
// can wedge in a state register dumps can't see. Only the REG00 soft-reset
// sequence (0x1F -> 0x00 -> 0x80) truly clears it; rewriting dividers doesn't.
static esp_err_t es8311_program(void) {
  esp_err_t err;
  ...
  // Record path: the Freenove S3 has a real onboard analog mic on the ADC
  // input (the board's stock firmware uses it for voice chat). Values match
  // Espressif's es8311 component's microphone config: REG14 selects the
  // analog mic with max PGA, REG16 sets mic gain (30dB), REG17 the ADC
  // digital volume. Harmless on boards with nothing wired to the ADC — the
  // RX I2S channel only runs while a recording is in flight.
  err = es8311_reg_write(0x14, 0x1A);
  if (err != ESP_OK) {
    return err;
  }
  // Mic gain one step down, 30dB -> 24dB (Jon 8/12: close/loud speech came
  // back "really distorted" — 57 clipped samples at 30dB slammed the ADC and
  // Whisper couldn't parse it). 24dB keeps a kid across the room clearly
  // audible while giving a close talker 6dB of headroom before clipping.
  err = es8311_reg_write(0x16, 0x04);
  if (err != ESP_OK) {
    return err;
  }
  err = es8311_reg_write(0x17, 0xC8);
  if (err != ESP_OK) {
    return err;
  }
  // ALC (automatic level control) OFF, explicitly (8/12). This bank was never
  // programmed, so it floated at chip defaults — and an active ALC moves the
  // ADC gain ON ITS OWN between captures: railed up in silence (the "hot"
  // 3x-gain clips with a 5k noise floor), clamped after loud input (the
  // "quiet" phase), hunting in between (garble). Registers we dumped looked
  // identical because the ALC bank was never in the dump. A wish recorder
  // wants one fixed, predictable gain: ALC disabled, soft-ramp off.
  err = es8311_reg_write(0x15, 0x00);   // ADC soft-ramp/DMIC defaults, ramp off
  if (err != ESP_OK) {
    return err;
  }
  err = es8311_reg_write(0x18, 0x00);   // ALC enable bit clear = manual gain
  if (err != ESP_OK) {
    return err;
  }
...
// Public: full codec soft-reset + reprogram, for the wish recorder to run
// before a capture attempt. Returns 0 on success. Safe no-op before init.
int dac_es8311_capture_reset(void) {
  if (s_es8311_device == NULL) {
    return -1;
  }
  if (s_dac_mutex) {
    xSemaphoreTake(s_dac_mutex, portMAX_DELAY);
  }
  esp_err_t err = es8311_program();
  if (s_dac_mutex) {
...
static void es8311_set_volume(float volume_airplay_db) {
  ...
  // ONE-TO-ONE IN DECIBELS. AirPlay hands over a 30 dB range; the original `* 2.0f` stretched
  // that across 60 dB of codec range, so every nudge of the phone's slider moved twice as far as
  // it should and the control felt wrong at every position except the very top. A dB is a dB —
  // mapping them 1:1 is what makes this behave like any other AirPlay speaker.
  //
  // MAX_VOLUME is therefore simply the codec level that the phone's FULL volume corresponds to,
  // and the bottom of the slider lands 30 dB below it.
  float db = (float)CONFIG_ES8311_MAX_VOLUME + volume_airplay_db;
  if (db < -95.5f) {
    db = -95.5f;
  }
  if (db > 32.0f) {
  ...
  ESP_LOGI(TAG, "Volume: %.1f dB AirPlay -> %.1f dB DAC -> reg 0x%02X",
